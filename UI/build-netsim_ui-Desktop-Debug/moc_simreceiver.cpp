/****************************************************************************
** Meta object code from reading C++ file 'simreceiver.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.2.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../netsim_ui/simreceiver.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'simreceiver.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.2.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_SimReceiver_t {
    QByteArrayData data[9];
    char stringdata[124];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    offsetof(qt_meta_stringdata_SimReceiver_t, stringdata) + ofs \
        - idx * sizeof(QByteArrayData) \
    )
static const qt_meta_stringdata_SimReceiver_t qt_meta_stringdata_SimReceiver = {
    {
QT_MOC_LITERAL(0, 0, 11),
QT_MOC_LITERAL(1, 12, 13),
QT_MOC_LITERAL(2, 26, 0),
QT_MOC_LITERAL(3, 27, 13),
QT_MOC_LITERAL(4, 41, 14),
QT_MOC_LITERAL(5, 56, 22),
QT_MOC_LITERAL(6, 79, 26),
QT_MOC_LITERAL(7, 106, 11),
QT_MOC_LITERAL(8, 118, 4)
    },
    "SimReceiver\0status_update\0\0current_frame\0"
    "current_window\0receiver_status_update\0"
    "something_need_to_announce\0const char*\0"
    "work\0"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_SimReceiver[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       3,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    2,   34,    2, 0x06,
       5,    1,   39,    2, 0x06,
       6,    1,   42,    2, 0x06,

 // slots: name, argc, parameters, tag, flags
       8,    0,   45,    2, 0x0a,

 // signals: parameters
    QMetaType::Void, QMetaType::Int, QMetaType::Int,    3,    4,
    QMetaType::Void, QMetaType::Int,    3,
    QMetaType::Void, 0x80000000 | 7,    2,

 // slots: parameters
    QMetaType::Void,

       0        // eod
};

void SimReceiver::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        SimReceiver *_t = static_cast<SimReceiver *>(_o);
        switch (_id) {
        case 0: _t->status_update((*reinterpret_cast< int(*)>(_a[1])),(*reinterpret_cast< int(*)>(_a[2]))); break;
        case 1: _t->receiver_status_update((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 2: _t->something_need_to_announce((*reinterpret_cast< const char*(*)>(_a[1]))); break;
        case 3: _t->work(); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        void **func = reinterpret_cast<void **>(_a[1]);
        {
            typedef void (SimReceiver::*_t)(int , int );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&SimReceiver::status_update)) {
                *result = 0;
            }
        }
        {
            typedef void (SimReceiver::*_t)(int );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&SimReceiver::receiver_status_update)) {
                *result = 1;
            }
        }
        {
            typedef void (SimReceiver::*_t)(const char * );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&SimReceiver::something_need_to_announce)) {
                *result = 2;
            }
        }
    }
}

const QMetaObject SimReceiver::staticMetaObject = {
    { &QObject::staticMetaObject, qt_meta_stringdata_SimReceiver.data,
      qt_meta_data_SimReceiver,  qt_static_metacall, 0, 0}
};


const QMetaObject *SimReceiver::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *SimReceiver::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_SimReceiver.stringdata))
        return static_cast<void*>(const_cast< SimReceiver*>(this));
    return QObject::qt_metacast(_clname);
}

int SimReceiver::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 4)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 4;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 4)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 4;
    }
    return _id;
}

// SIGNAL 0
void SimReceiver::status_update(int _t1, int _t2)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)), const_cast<void*>(reinterpret_cast<const void*>(&_t2)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}

// SIGNAL 1
void SimReceiver::receiver_status_update(int _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}

// SIGNAL 2
void SimReceiver::something_need_to_announce(const char * _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 2, _a);
}
QT_END_MOC_NAMESPACE
